...
void pinSetup::configurePins(){
    ...
    for(a=0; a<sizeof(INTERLINE)/sizeof(INTERLINE[0]); a++){
      ...
      analogWriteFrequency(INTERLINE, LED_FREQ); //Set output PWM freq for LEDs https://www.pjrc.com/teensy/td_pulse.html
    } 
    ...
    for(a=0; a<sizeof(FAN_PWM)/sizeof(FAN_PWM[0]); a++){
      ...
      analogWriteFrequency(FAN_PWN[a], FAN_FREQ); //Set output PWM freq to optimal CPU fan freq, also sets analog_select PWM freq (on same timer): https://www.pjrc.com/teensy/td_pulse.html
    } 
